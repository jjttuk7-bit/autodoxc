...
> **마지막 갱신**: 2026-06-28
> **메인 브랜치 HEAD**: `b4e15e6`+ — 데이터 기반 시드 레지스트리 + 행정 구제·신청 시드 2종 (prod 0.0.12)
...
| **시드 doc_type** | 9종 — 레거시 3종(외국인 고용·내용증명·행정심판 청구서) + 데이터 시드 6종(정보공개 청구서·사업자등록 신청서·이의신청서(지방세)·행정심판 답변서·영업신고서·처분사전통지 의견제출서). 신규는 `app/agents/seed_docs.py`에 데이터로 선언. 키워드: 데이터 시드가 레거시보다 우선(행정심판 답변 vs 행정심판) | 50선 계속 확충 (계약서류·소송서류 등) |
