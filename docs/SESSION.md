...
| **시드 doc_type** | 5종 — 레거시 3종(외국인 고용·내용증명·행정심판) + 데이터 시드 2종(정보공개 청구서·사업자등록 신청서). 신규는 `app/agents/seed_docs.py`에 데이터로 선언(확장 쉬움). 레거시 3종은 추후 데이터로 이관 | 50선 중 행정 구제·신청 P0 추가 진행 중 |
